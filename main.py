...
from models.resnet_49 import resnet  # The model construction
from opts import opts  # The options for the project
from trainer_new import train  # For the training process
from trainer_new import validate  # For the validate (test) process
from trainer_new import adjust_learning_rate
from trainer_new import vis
...
def main():
    ...
    proto = torch.zeros((args.num_classes, feat_size)).cuda()
    ...
    if args.arch == 'alexnet':
        ...
        optimizer = torch.optim.SGD([
            ...
            {'params': model.module.rl_rnn.parameters(), 'name': 'pre-trained'},
            {'params': model.module.linear_rl_image_mus.parameters(), 'name': 'pre-trained'},
            {'params': model.module.bn.parameters(), 'name': 'new-added'},
            {'params': model.module.linear_last.parameters(), 'name': 'new-added'},
            {'params': model.module.dis_linear.parameters(), 'name': 'new-added'},
            {'params': model.module.dis_classify.parameters(), 'name': 'new-added'},
    ...
    #test only
    if args.test_only:
        featuer_ori = cul_anchor_feature(model)
        cul_anchor_target_sim(val_loader, model, featuer_ori, -1, args)
    ...
    while (current_epoch < args.epochs):
        ...
        source_train_loader_batch, target_train_loader_batch, current_epoch, new_epoch_flag, iteration = \
            train(iteration, proto, source_train_loader, source_train_loader_batch, target_train_loader,
                  target_train_loader_batch, model, criterion_classifier_source, criterion_classifier_target,
                  criterion_em_target, criterion, optimizer, current_epoch, epoch_count_dataset, args)
...
def save_checkpoint(state, is_best, args, epoch):
    # filename = 'checkpoint_{}.pth.tar'.format(str(epoch))
