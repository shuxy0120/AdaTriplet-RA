...
from trainer_new import MMD_loss
...
def train(iteration, proto, source_train_loader, source_train_loader_batch, target_train_loader,
    ...
    ############################################ for source samples
    output_source, feature_source = model(input_source_var)
    ...
    loss_category_st_G = 0.5 * criterion(output_source, target_source_var) + 0.5 * criterion(output_source,
                                                                                             target_source_temp_var)

    input_target_var = torch.autograd.Variable(input_target)
    output_target, feature_target = model(input_target_var)
    ...
    # mmd_loss = MMD_loss()(feature_source, feature_target)
    ...
    if args.flag == 'no_em':
        ...
        loss_classifier = loss_task_s_Cs + loss_task_s_Ct + loss_domain_st_Cst_part1 + loss_domain_st_Cst_part2 # + mmd_loss  ### used to classifier
